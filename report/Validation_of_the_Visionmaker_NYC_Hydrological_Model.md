...
climate scenario, precipitation events, and land-use types as
parameters, collectively referred to as a “Vision” for the city. The
model then predicts water usage, biodiversity, greenhouse gas emission,
...
World NYC (ESW-NYC) to assess Visionmaker’s water model. The water model
is a simplified representation of the hydrological system in NYC. The
flowchart below shows how Visionmaker relates different variables
(Figure 1).[^2]

<center>![Visionmaker models hydrology in NYC as a simple flow-through
...
determines the amount of runoff.](flowchart.jpg)</center>

In a “black box” approach to validation, we focused on evaluating a
model output and a model input. Street flooding, called
“Undifferentiated Floodwaters” in Visionmaker, is compared to actual
resident reports of flooding during large rain storms. The rain storms
themselves, called “rain events,” are compared to other sources of
...
hydrological model contained within Visionmaker NYC against observed
...
of when and where in NYC flooding occurred 2015. Daily historical
...
Visionmaker NYC, producing a predicted volume of flood water. Comparing
whether Visionmaker NYC predictions of flooding coincide with known
...
Data on 311 reports from 2015 was obtained from NYC Open Data
...
the flooded area in Visionmaker NYC. Flooding events were modeled in
Visionmaker NYC as follows, making reference to the columns of the
spreadsheet
`Visionmaker_modelling_results``/Visionmaker_flood_predictions.csv`:

1.  After opening Visionmaker NYC and logging in, follow the menus and
...
2.  Since the flooding events are marked by longitude and latitude, but
    Visionmaker NYC does not provide a search functionality for
...
3.  The aggregated precipitation of the flooding event is next converted
    to the appropriate Visionmaker NYC parameters. Internally, the
    Visionmaker NYC model uses the product of two parameters, storm
    duration and storm intensity, to determine the total precipitation
    of a storm. Since Visionmaker NYC uses a “bucket model” in
...
Note the outline of New York City formed by the flood reports as well as
the darker red points where two flood events coincide.](../outputs/flood_events_and_reports_by_location.png)
</center>
...
events, future work might also consider elevation and topography data
where we would expect only low-lying areas to flood. These checks give
...
Visionmaker NYC model did not predict flooding at any of the flood
...
Visionmaker NYC uses a "bucket model" to calculate floodwaters: whatever
...
-   Allow more precise or direct manipulation of total rainfall in
    ...
    discontinuities; see
    `Visionmaker_modelling_results``/Visionmaker_storm_parameters.csv`.
...
inches/hour. Duration is how long the storm event lasts in hours. Return
...
any year and occurs once every five years on average.
...
indicated by the red stars. As for the other two storm types, “Clear
Day” has zero rainfall and does not require analysis, and “Showers” was
not derived empirically but as an *ad hoc* estimation.

<center>
![This IDF curve was published in a climate change impact report
by the DEP of NYC in 2008. From left to right, the stars correspond to
“Thunderstorm,” “Rainy Day,” “Soaking Storm,” and “Severe Storm” rain
events.[^11]](idf_curve.png)
</center>

Visionmaker projects rain intensities for future climate scenarios using
baseline data from the Figure 4 IDF curve. Future climate scenarios are
scaled from the baseline by fixed percentages published by the NYC Panel
...
----------------------
The NRCC and Visionmaker datasets broadly match in that short storms are
...
scenarios using fixed percentages of the baseline intensities from the
...
types. However, the methodology used by Visionmaker deviates from
standard statistical practice. The fixed percentages in the NPCC reports
are meant to represent changes to average total annual precipitation. An
average total does not necessarily translate to the intensity of an
individual storm. Visionmaker is currently set to model everything on
June 1^st^, meaning that snow and other annual variation (i.e. seasons)
should not count towards Visionmaker’s calculations.

It should be noted that Visionmaker’s 2100s projections have lower
intensities and total rainfalls than the 2080s. A more recent NPCC
report was used with different periods of time predicted and different
percent increases. Climate predictions can vary widely between different
models, which is one reason that using a single source model
consistently is important. We recommend that Visionmaker shift to using
a single source for projections, especially one that

Visionmaker uses 1609, the year the Dutch began exploring NYC for
...
the 1903-1950 baseline intensity by 10%. This projection needs more
substantiated evidence. It is not obvious that a drought in Virginia
would reach New York. Further, extreme droughts are characterized by
larger drops than 10%.[^17]
...
-   Choose a return period and data source for the 2-hour storm type
    “Shower.”
...
-   If the DEP IDF curve is retained, update “baseline” time period from
...
[^8]: `Visionmaker_modelling_results``/Visionmaker_storm_parameters.csv`

[^9]: `outputs_1/311_2015_floods.csv`

[^10]: `Visionmaker_modelling_results``/Visionmaker_flood_predictions.csv`
...
[^14]: This emission scenario corresponds to the higher model of
    ...
    Climate Change: 1200 ppm of CO2 equivalent by 2100, or four times
